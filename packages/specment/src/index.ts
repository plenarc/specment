...
async function listDirectory(dir: string, prefix: string) {
...
async function validateDirectory(dir: string, config: any, fix: boolean): Promise<number> {
  const items = await fs.readdir(dir);
  let issueCount = 0;
...
async function validateFile(filePath: string, config: any, fix: boolean): Promise<number> {
  ...
  if (config.naming) {
    ...
    const isException = exceptions.some((exception: string) => {
    ...
    if (!isException) {
      ...
      if (pattern === 'flexible' && allowedPatterns.length > 0) {
        ...
        isValidNaming = allowedPatterns.some((allowedPattern: string) => {
